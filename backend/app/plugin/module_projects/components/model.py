...
from app.plugin.module_orders.order.model import OrdersModel
...
class ComponentsModel(ModelMixin, UserMixin):
    ...
    project_code: Mapped[str] = mapped_column(String(64), ForeignKey("projects_projects.code"), comment="项目编码")
    ...
    material:Mapped[str | None] = mapped_column(String(255), comment="材料")
    ...
    # 关联明细
    parts: Mapped[List["PartsModel"]] = relationship("PartsModel", back_populates="component", cascade="all, delete-orphan")
    
    # 关联工单
    order: Mapped["OrdersModel"] = relationship("OrdersModel", back_populates="component", uselist=False)
